...
class Container {
    ...
    def test2(image) {
        ...
        steps.sh "docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v `pwd`:/project -e SNYK_TOKEN=${token} -e MONITOR=false snyk/snyk-cli:docker test --docker --severity-threshold=high  --policy-path=policy ${image}"
    }
